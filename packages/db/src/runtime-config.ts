...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "openblock-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultConfigPath(): string {
  return path.resolve(
    resolveOpenblockHomeDir(),
    "instances",
    resolveOpenblockInstanceId(),
    CONFIG_BASENAME,
  );
}

function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveOpenblockHomeDir(), "instances", resolveOpenblockInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".openblock", CONFIG_BASENAME);
...
function resolveOpenblockConfigPath(): string {
  if (process.env.OPENBLOCK_CONFIG?.trim()) {
    return path.resolve(process.env.OPENBLOCK_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveOpenblockEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveOpenblockEnvPath(configPath);
  ...
  const port = config?.database?.embeddedPostgresPort ?? 54331;
