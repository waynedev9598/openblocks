...
function resolveOpenblockInstanceId(): string {
  const raw = process.env.OPENBLOCK_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveOpenblockHomeDir(), "instances", resolveOpenblockInstanceId(), "config.json");
}
...
function resolveConnectionString(config: PartialConfig | null): string {
  ...
  return `postgres://openblock:openblock@127.0.0.1:${port}/openblock`;
}

function resolveDefaultBackupDir(): string {
  return path.resolve(resolveOpenblockHomeDir(), "instances", resolveOpenblockInstanceId(), "data", "backups");
}
