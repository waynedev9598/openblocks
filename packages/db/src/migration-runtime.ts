...
async function ensureEmbeddedPostgresConnection(
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://openblock:openblock@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "openblock");
    return {
      connectionString: `postgres://openblock:openblock@127.0.0.1:${port}/openblock`,
  ...
  const adminConnectionString = `postgres://openblock:openblock@127.0.0.1:${preferredPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "openblock");

  return {
    connectionString: `postgres://openblock:openblock@127.0.0.1:${preferredPort}/openblock`,
